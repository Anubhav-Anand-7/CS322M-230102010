...
It is special in the sense that 10 new instructions RVX10 apart from RV32I core. 
...
- `riscvsingle.sv` — SystemVerilog source with the **CPU + testbench**. The testbench top module is `testbench` and instantiates `top` (the CPU). It contains the **10 newly added RVX10 instructions**.
- `riscvtest.txt` — **Instruction memory image** loaded by `$readmemh` (one 32‑bit hex word per line).
- `riscvtest.s` —  **Assembly** source corresponding to `riscvtest.txt` (for reference).
